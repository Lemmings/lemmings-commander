...
var scan = function(self, dir, tacticsHandler, config, callback){
...
console.log('strategy read,%s', dir + '/' + file);
            self.strategys[file] = new Strategy(tacticsHandler, file, data, config['strategy/'+file]);
            scancallback(err);
        });
    }, function(err){
        if(err){
...
StrategyManager.prototype.initialize = function(tacticsHandler, config, callback){
    var self = this;
    var tasks = [];
    this.dir.forEach(function(dir){
        tasks.push(function(next){
            scan(self, dir, tacticsHandler, config, function(err){
                next(err);
            });
        });
    });
